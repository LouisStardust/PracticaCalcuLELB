...
const App = () => {
  ...
  const calculateResult = () => {
    try {
      ...
      // Verificación para la raíz cuadrada de números negativos
      const negativeSqrtMatch = modifiedInput.match(/sqrt\(-\d+\)/);
      if (negativeSqrtMatch) {
        setResult('No existe'); // Mensaje personalizado para raíces negativas
        return;
      }
  
      let evaluatedResult = math.evaluate(modifiedInput);
  
      // Verificación para la división por cero
